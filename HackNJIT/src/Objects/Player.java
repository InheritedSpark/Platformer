...
public class Player extends Object {
	...
	public void tick() {
		...
		fall();
		build();
		throwBomb(handler.getGrenade(), 32, 96, .75f);
		if (health <= 0) {
	...
	public void render(Graphics g) {
		...
		if (buildMode) {
			...
			if (highlightPlatform) {
				...
				g2d.fill(new Rectangle(handler.getMouse()[0] / platWidth * platWidth,
						handler.getMouse()[1] / platHeight * platHeight, platWidth, platHeight));
			}
		}
	}
			...
			if (handler.isMouse1()) {
				...
				if (!crossing) {
					if (getBoundsBuild().intersects(new Rectangle(platformX, platformY, 32, 32))) {
						gameObjects.add(
								new Platform(game, platformX, platformY, platWidth, platHeight, ID.Platform, true));
					}
				}
			}
		}
	...
	public void throwBomb(int shoot, int bomb_radius, int range, float timer){
		if(shoot == 0){
			handler.grenade = -1;
			handler.addObject(new Grenade(game, x, y-bomb_radius, bomb_radius, bomb_radius, facing, -1, "Bomb", 200, range, -1, -1, ID.Grenade, handler,timer));
		}
	}
