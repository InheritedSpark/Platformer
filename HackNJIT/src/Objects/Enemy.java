	...
	public void tick() {
		if (health <= 0) { // remove enemy when killed
			...
			if (death.getCurrentFrame() == game.asset.enemy1Death[game.asset.enemy1Death.length - 1]) {
				...
				for (int i = 0; i < 1; i++) {
					game.handler.addObject(new HealthParticle(game, game.handler.player, x + ran.nextInt(20) + 1,
							y + ran.nextInt(20) + 1, 16, 16, new Color(14, 232, 221), ID.Effect));
	...
	public void render(Graphics g) {
		...
		case 0:
			if (!flash) {
				...
				if (counter <= 2) {
					if (currentvx >= 0) {
						g2d.drawImage(game.asset.dye(walk.getCurrentFrame(), new Color(255 * c, 255 * c, 255 * c)),
								(int) x, (int) y, width, height, null);
					} else {
						g2d.drawImage(game.asset.dye(walk.getCurrentFrame(), new Color(255 * c, 255 * c, 255 * c)),
								(int) x + width, (int) y, -width, height, null);
					}
				}
			}
			break;
