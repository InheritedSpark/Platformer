...
public class Handler {
	...
	private LinkedList<ArrayList<Tile>> layers;
	...
	public Handler(Main game) {
		...
		layers = new LinkedList<ArrayList<Tile>>();
	...
	public void tick() {
		if(!restart){
			...
			for (int i = 0; i < objects.size(); i++){
				...
				if(obj.getBounds().intersects(cam.getBounds2())){ // check if object is in camera
					for (int j = 0; j < gameObjects.size(); j++){ // find the object
						if(obj == gameObjects.get(j)){ // dont add if found and break from the loop
							found = true;
							break;
						}
					}
			...
			for (int i = 0; i < gameObjects.size(); i++) {
				...
				if(!gameObj.getBounds().intersects(cam.getBounds())){	// outside the camera 
...
//				}
				if(gameObj.getId() == ID.Portal){
					...
					for(int k = 0; k<mp.path.length; k++){
						if(new Rectangle((int)mp.path[k][0], (int)mp.path[k][1], 10, 10).intersects(cam.getBounds())){
							draw = true;
							break;
						}
					}
...
//		}
		...
		//g2d.translate(-((game.Width/2)-(cam.camXCenter*cam.zoomRatio)+shake), -((game.Height/2)-(cam.camYCenter*cam.zoomRatio)+shake));
...
//		player = new Player(game, 2*32,14*32,64,64,ID.Player1,this);
		...
		addObject(new Portal(game, 81*32, 33*32,85*32, 32*32, 32, 32, ID.Portal));
...
//		addObject(new Sniper(game, game.Height/3  + 128, 100, 80, 50, ID.Gun, "sniper"));
//		addObject(new Ak47(game, game.Height / 2 - 110, 100, 60, 32, ID.Gun, "ak47"));
//		addObject(new Uzi(game, game.Height / 4 - 110, 100, 50, 32, ID.Gun, "uzi"));
//		addObject(new MachineGun(game, game.Height / 5 - 110, 100, 50, 32, ID.Gun, "lmg"));
	}
