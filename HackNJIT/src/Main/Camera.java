...
public class Camera {
	...
	public void render(Graphics g) {
		...
		g2d.draw(new Rectangle((int) camXCenter - safeAreaX / 2, (int) camYCenter - safeAreaY / 2, safeAreaX,
				safeAreaY));
	}
	...
	public void center(Objects.Object target) {
		...
		else if ((target.getBounds().x + target.getBounds().width) >= game.Width / 2) { // center
																						// on
																						// target
																						// only
																						// when
		...
		if (target.getBounds().y + target.getBounds().height - game.Height / 2 >= (handler.gameHeight * 32)
				- game.Height) {
